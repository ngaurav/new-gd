...
# This class is a custom logging filter that prevents repeated log messages from being displayed.
...
async def download_upscaled_images(page, prompt_text: str):
    ...
    try:
        ...
        midjourney_url_files = 'midjourney/mid_journey_images.txt'
        # Clear the txt file before writing new URLs
        with open(midjourney_url_files, 'w') as f:
            f.write("")
        ...
        async with aiohttp.ClientSession() as session:
            for i,url in enumerate(image_urls):
                async with session.get(url) as resp:
                    if resp.status == 200:
                        with open(midjourney_url_files, 'a') as f:
                            f.write(url + "\n")
                        if not os.path.exists('midjourney/mid_journey_images'):
                            os.makedirs('midjourney/mid_journey_images')
                        file_name = "midjourney/mid_journey_images/image_"+str(i+1)+".png"
...
async def send_bot_command(page, command: str, PROMPT: str):
    ...
    try:
        ...
        chat_bar = page.get_by_role('textbox', name='Message #dreamland')
        await asyncio.sleep(random.randint(1, 5))
...
def call_midjourney(prompt: str, width: int, height: int) -> None:
    check_creds()
    bot_command = "/imagine"
    channel_url = "https://discord.com/channels/1224572672554303498/1224586563510468619"
    prompt = prompt + f" --ar {width}:{height}"
    asyncio.run(main(bot_command, channel_url, prompt))
...
if __name__ == '__main__':
    ...
    password = os.environ.get("DISCORD_PASSWORD")
    check_discord_credentials(email, password)
    prompt_file_path = "openai/gpt4_response/gpt4_response.json"    
    with open(prompt_file_path, 'r') as file:
        prompt = json.load(file)
    logo_descriptions_without_text = prompt["Logo_Component"]["logo_generation_prompt"]
